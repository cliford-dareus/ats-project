...
import {
    ...
    Calendar, CheckCircle, CheckCircle2, ChevronDown, Clock,
    Edit3,
    Expand, Mail, Maximize2, Paperclip, Phone,
    Share,
    Share2, Star,
    Tally1, UserCheck,
    Users, UserX,
...
const ApplicationPreview = ({data}: Props) => {
                ...
                <div className="flex-1 overflow-y-auto space-y-4">
                    ...
                    <div className="bg-foreground rounded-3xl p-8 text-white relative overflow-hidden">
                        <div className="relative z-10 space-y-6">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold font-mono uppercase tracking-tight">{data.candidate_name}</h3>
                                <p className="text-xs font-bold text-white/40 uppercase tracking-widest mt-1">Applying for <span className="uppercase text-primary">{data.job_apply}</span></p>
                            </div>

                            <div className="grid grid-cols-2 gap-y-4 pt-4 border-t border-white/10">
                                <div>
                                    <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Rating</p>
                    ...
                    {/* Quick Actions */}
                    <div className="flex items-center justify-between">
                        <h4 className="text-xs font-bold text-brand-dark uppercase tracking-widest">QUICK ACTIONS</h4>
                        <div className="flex gap-2">
                            ...
                            <div className="flex">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent defaultValue={data.current_stage} align="end">
                                        <DropdownMenuLabel>Advance Candidate</DropdownMenuLabel>
                                        <DropdownMenuSeparator />
                                        {JOB_STAGES.map(stage => (
                                            <DropdownMenuItem defaultValue={stage === data.current_stage?data.current_stage: ""} key={stage}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                            <div className="flex items-center gap-2 text-zinc-400 mb-1">
                                ...
                                <span className="text-xs font-bold uppercase text-foreground/30 tracking-wider">Current Stage</span>
                            </div>
                            <p className="text-xl font-bold">{data.current_stage}</p>
                        </div>
                        <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                            <div className="flex items-center gap-2 text-zinc-400 mb-1">
                                <Calendar className="w-4 h-4"/>
                                <span className="text-xs font-bold uppercase text-foreground/30 tracking-wider">Applied On</span>
                            </div>
                            <p className="text-xl font-bold">{new Date(data.apply_date).toDateString()}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-4 group">
                            <div className="w-8 h-8 rounded-lg bg-background flex items-center justify-center text-brand-dark/30 group-hover:text-primary transition-colors">
                                <Mail size={16} />
                            </div>
                            <div className="flex-1">
                                <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">EMAIL</p>
                                <p className="text-xs font-bold">{data.candidate_email}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 group">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs font-bold">{data.candidate_phone}</p>
                            </div>
                        </div>

                        <div className="flex items-center gap-4 group">
                            <div className="w-8 h-8 rounded-lg bg-background/5 flex items-center justify-center text-brand-dark/30 group-hover:text-primary transition-colors">
                                <Paperclip size={16} />
                            </div>
                            <div className="flex-1">
                                <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">ATTACHMENT</p>
                                <p className="text-xs font-bold text-primary hover:underline cursor-pointer">attachment.pdf</p>
                            </div>
                            <div className="flex gap-2">
                        ...
                        <div className="flex items-center gap-4 group">
                            ...
                            <div className="flex-1">
                                <p className="text-[10px] font-bold text-foreground/30 uppercase tracking-widest">CAN CONTACT</p>
                    ...
                    <InternalNoteSection parent_type="application" parent_id={data.id}  selectedId={data.id}/>
                </div>
            </div>
        </>
    );
