  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {!applicationResult.interview ? (
            ...
            <span className="px-2 py-0.5 bg-zinc-100 text-zinc-500 text-[10px] font-bold rounded-full border border-zinc-200 uppercase tracking-wider">
              Not Booked
            </span>
          )}
        </div>
        ...
        {applicationResult.interview ? (
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-zinc-900">
                    ...
                    {applicationResult.interview.link
                      ? "via Link"
                      : `at ${applicationResult.interview.locations}`}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="text-center py-6 bg-zinc-50 rounded-xl border border-dashed border-zinc-200">
            ...
            <button onClick={() => setIsOpen(true)} className="px-4 py-2 bg-white border border-zinc-200 rounded-lg text-xs font-bold text-accent hover:bg-zinc-50 transition-all shadow-sm">
              Schedule Interview
            </button>
          </div>
        )}
      ...
      <ScheduleInterviewModal
        ...
        jobDetails={{jobName: applicationResult.job_apply, department: applicationResult.department}}
      />
    </>
  );
};
